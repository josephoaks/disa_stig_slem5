...
install_slem_patches() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        sudo reboot
        log_message "$function_name" "$vuln_id" "$rule_id" "System is rebooting to apply patches."
    else
...
install_kbd_package() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        sudo reboot
        log_message "$function_name" "$vuln_id" "$rule_id" "System is rebooting to apply the changes."
    else
